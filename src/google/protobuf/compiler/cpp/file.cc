...
std::vector<int> FileGenerator::GetTopLevelMessageIndices() const {
  std::vector<int> indices;
  for (size_t i = 0; i < message_generators_.size(); ++i) {
    const Descriptor* desc = message_generators_[i]->descriptor();
    // Top-level messages have no containing type
...
int FileGenerator::NumTopLevelMessages() const {
  return static_cast<int>(GetTopLevelMessageIndices().size());
}

void FileGenerator::GenerateForwardDeclarationHeader(io::Printer* p) {
  auto v = p->WithVars(FileVars(file_, options_));
  std::string guard = absl::StrCat(
      absl::AsciiStrToUpper(absl::StrReplaceAll(
          absl::StrCat(StripProto(file_->name()), "_FWD_PB_H_"),
          {{"/", "_"}, {".", "_"}, {"-", "_"}})));
  ...
  p->Print(
      ...
      "GENCODE\n"
      "// source: $filename$\n"
      "// Forward declarations for modular generated code.\n");
  if (options_.opensource_runtime) {
    p->Print("// Protobuf C++ Version: $protobuf_cpp_version$\n",
             "protobuf_cpp_version", PROTOBUF_CPP_VERSION_STRING);
  }
  ...
  // Generate forward declarations for all messages
  {
    NamespaceOpener ns(Namespace(file_, options_), p);
    std::vector<const Descriptor*> classes;
    FlattenMessagesInFile(file_, &classes);
    for (const Descriptor* desc : classes) {
      p->Emit({{"classname", ClassName(desc, false)}}, R"cc(
        class $classname$;
      )cc");
    }
  }
...
void FileGenerator::GenerateModularMessageHeader(int idx, io::Printer* p,
                                                 absl::string_view info_path) {
  std::vector<int> top_level_indices = GetTopLevelMessageIndices();
  if (idx < 0 || idx >= static_cast<int>(top_level_indices.size())) {
    return;
  }

  int msg_idx = top_level_indices[idx];
  const Descriptor* desc = message_generators_[msg_idx]->descriptor();
  std::string msg_name = desc->name();

  auto v = p->WithVars(FileVars(file_, options_));
  std::string guard = absl::StrCat(
      absl::AsciiStrToUpper(absl::StrReplaceAll(
          absl::StrCat(StripProto(file_->name()), "_", msg_name, "_PB_H_"),
          {{"/", "_"}, {".", "_"}, {"-", "_"}})));
  ...
  // Include forward declarations for this file
  std::string fwd_include = absl::StrCat(StripProto(file_->name()), "_fwd.pb.h");
  p->Emit({{"fwd_include", fwd_include}}, R"(
    #include "$fwd_include$"
  )");

  // Include dependencies from other files that this message needs
  GenerateDependencyIncludes(p);

  // Include port definitions
  IncludeFile("third_party/protobuf/port_def.inc", p);

  p->Emit({{"dllexport_macro", FileDllExport(file_, options_)}}, R"(
    #define $dllexport_macro$$ dllexport_decl$
  )");
  ...
  // Generate the message definition
  {
    ...
    // Generate any enums defined within this message
    for (size_t i = 0; i < enum_generators_.size(); ++i) {
      const EnumDescriptor* enum_desc = enum_generators_[i]->descriptor();
      if (enum_desc->containing_type() == desc) {
        enum_generators_[i]->GenerateDefinition(p);
      }
    }

    // Generate the message class definition
    p->Emit(R"cc(
      $hrule_thin$
    )cc");
    message_generators_[msg_idx]->GenerateClassDefinition(p);

    // Generate any nested messages
    for (size_t i = 0; i < message_generators_.size(); ++i) {
      const Descriptor* nested_desc = message_generators_[i]->descriptor();
      if (nested_desc->containing_type() == desc) {
        p->Emit(R"cc(
          $hrule_thin$
        )cc");
    ...
    // Generate inline methods for the message
    p->Emit(R"(
      #ifdef __GNUC__
      #pragma GCC diagnostic push
      #pragma GCC diagnostic ignored "-Wstrict-aliasing"
    ...
    message_generators_[msg_idx]->GenerateInlineMethods(p);

    // Generate inline methods for nested messages
    for (size_t i = 0; i < message_generators_.size(); ++i) {
      const Descriptor* nested_desc = message_generators_[i]->descriptor();
      if (nested_desc->containing_type() == desc) {
        message_generators_[i]->GenerateInlineMethods(p);
...
void FileGenerator::GenerateModularUmbrellaHeader(io::Printer* p,
                                                  absl::string_view info_path,
                                                  absl::string_view basename) {
  auto v = p->WithVars(FileVars(file_, options_));
  std::string guard = absl::StrCat(
      absl::AsciiStrToUpper(absl::StrReplaceAll(
          absl::StrCat(basename, "_PB_H_"),
          {{"/", "_"}, {".", "_"}, {"-", "_"}})));
  ...
  p->Print(
      ...
      "GENCODE\n"
      "// source: $filename$\n"
      "// Umbrella header for modular generated code.\n");
  if (options_.opensource_runtime) {
    p->Print("// Protobuf C++ Version: $protobuf_cpp_version$\n",
             "protobuf_cpp_version", PROTOBUF_CPP_VERSION_STRING);
  }
  ...
  // Include the forward declarations header
  std::string fwd_include = absl::StrCat(basename, "_fwd.pb.h");
  p->Emit({{"fwd_include", fwd_include}}, R"(
    #include "$fwd_include$"  // IWYU pragma: export
  )");

  // Include all individual message headers
  std::vector<int> top_level_indices = GetTopLevelMessageIndices();
  for (int idx : top_level_indices) {
    const Descriptor* desc = message_generators_[idx]->descriptor();
    std::string msg_header = absl::StrCat(
        basename, "/", absl::AsciiStrToLower(desc->name()), ".pb.h");
    p->Emit({{"msg_header", msg_header}}, R"(
    #include "$msg_header$"  // IWYU pragma: export
    )");
  }

  // Include any file-level enums, services, and extensions that are not
  // part of a message. For full compatibility, we still generate these
  // in the umbrella header.

  // Library includes for file-level enums/services/extensions
  if (file_->enum_type_count() > 0 || file_->service_count() > 0 ||
      file_->extension_count() > 0) {
    GenerateLibraryIncludes(p);
    IncludeFile("third_party/protobuf/port_def.inc", p);
...

    {
      ...
      // Generate file-level enums
      for (int i = 0; i < file_->enum_type_count(); ++i) {
        for (size_t j = 0; j < enum_generators_.size(); ++j) {
          if (enum_generators_[j]->descriptor() == file_->enum_type(i)) {
            enum_generators_[j]->GenerateDefinition(p);
            break;
          }
        }
      }
      ...
      // Generate extension identifiers
      for (auto& extension_generator : extension_generators_) {
        if (extension_generator->IsScoped()) {
          continue;
        }
    ...
    // Generate GetEnumDescriptor specializations
    if (HasEnumDefinitions(file_)) {
      NamespaceOpener ns(ProtobufNamespace(options_), p);
      for (int i = 0; i < file_->enum_type_count(); ++i) {
        for (size_t j = 0; j < enum_generators_.size(); ++j) {
          if (enum_generators_[j]->descriptor() == file_->enum_type(i)) {
            enum_generators_[j]->GenerateGetEnumDescriptorSpecializations(p);
            break;
          }
        }
      }
